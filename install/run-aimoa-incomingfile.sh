...
# main version
cd $AIMOA/src
python3 main.py --config $AIMOA/config/config-incomingfile.yaml --workflow-config $AIMOA/config/workflow-config-incomingfile.yaml --reset-lock --cron-interval */1 --run-immediately
# (huey version)
